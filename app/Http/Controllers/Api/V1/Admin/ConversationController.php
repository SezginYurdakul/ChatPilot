...
class ConversationController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $query = Conversation::whereIn('site_id', $siteIds)
            ->with('site:id,name')
        ...
        $conversations->getCollection()->transform(function ($conversation) {
            $conversation->visitor_online = VisitorPresence::isOnline($conversation->id);
    ...
    public function update(Request $request, Conversation $conversation): JsonResponse
    {
        ...
        // Send a system message when conversation is closed or reopened
        if ($oldStatus !== $newStatus && in_array($newStatus, ['closed', 'active'])) {
     ...
     *
     * DELETE /api/v1/admin/conversations/{conversation}
     */
